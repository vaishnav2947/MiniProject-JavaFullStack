...
// Java enums come back UPPER_CASE — normalize for display lookup
const norm = (v) => (v || '').toLowerCase().replace('_', '-')

export const priorityConfig = {
  low:      { label:'Low',      color:'#10b981', bg:'rgba(16,185,129,0.1)',  border:'rgba(16,185,129,0.3)',  dot:'🟢' },
  medium:   { label:'Medium',   color:'#f59e0b', bg:'rgba(245,158,11,0.1)', border:'rgba(245,158,11,0.3)',  dot:'🟡' },
  high:     { label:'High',     color:'#f97316', bg:'rgba(249,115,22,0.1)', border:'rgba(249,115,22,0.3)',  dot:'🟠' },
  critical: { label:'Critical', color:'#ef4444', bg:'rgba(239,68,68,0.1)',  border:'rgba(239,68,68,0.3)',   dot:'🔴' },
}
...
export const Badge = ({ type, value }) => {
  ...
  const config = type === 'priority' ? priorityConfig[key]
               : type === 'status'   ? statusConfig[key]
               : typeConfig[key]
  if (!config) return (
    <span style={{ fontSize:11, color:'#4a5f85', padding:'2px 8px' }}>
      {value || '—'}
    </span>
  )
  return (
...
export const Avatar = ({ name = '', size = 32 }) => {
  ...
  const colors = ['#4f7cff','#10b981','#f59e0b','#8b5cf6','#ef4444','#06b6d4','#ec4899']
...
export const getPriorityColor = (p) => {
  const cfg = priorityConfig[(p||'').toLowerCase()]
  return cfg?.color || '#8899b8'
}
